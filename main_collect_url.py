...
#     ,'AOT','ADVANC']
#     ,'IVL','PTTEP','SCB','INTUCH','PTTGC','GULF']
        ...
        for tag1 in Set100:
                ...
                # df = pd.DataFrame(list(zip(stock_name,date,Title, p,l)), 
                ...
                p_1.append(p)
                l_1.append(l)
        # df = pd.DataFrame(list(zip(stock_name_1,date_1,Title_1, p_1,l_1)), 
        # columns =['stock_name','date','Title', 'link','source'])
        # print(p_1)
        ...
        #         dm=row['date'].split()
        #         dm1=date_month(dm[0],dm[1])
        #         print(dm1)
        #         # print(row['stock_name'], row['date'],row['Title'], row['link'], row['source'])
        #         if row['date']=='02 Dec 13:31':
