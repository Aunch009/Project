# Reference
# code:https://github.com/huggingface/notebooks/blob/main/examples/summarization.ipynb
...
def load_data(data_dir):
    ...
    df_concat1['all'] = df_concat1.apply(
        lambda x: x['group_document'] + '_' + x['filename'], axis=1)
    ...
    sample_train_dataset = Dataset.from_dict(
        {"document": train["document"].tolist(), "summary": train["summary"].tolist()})
    sample_valid_dataset = Dataset.from_dict(
        {"document": validate["document"].tolist(), "summary": validate["summary"].tolist()})
    print('load data')
    del df
    del train
    del test1
...
def pretrain_model(data_dir):
    ...
    train_datasets = sample_train_dataset.map(
...
if __name__ == "__main__":
    ...
    text_file.write("Text Class name:" +
                    str(os.listdir(args.data_path)) + '\n')
    text_file.write("News_Articles Class name:" +
                    str(os.listdir(os.path.join(args.data_path, 'News_Articles'))) +
                    '\n')
    text_file.write("Summaries Class name:" +
                    str(os.listdir(os.path.join(args.data_path, 'Summaries'))))
    text_file.close()
    init()
