...
def load_data(data_dir):
    ...
    df_concat['group_document_num'] = df_concat['group_document'].astype(
        "category").cat.codes
    ...
    df_all['group_document_num'] = df_all['group_document'].astype(
        "category").cat.codes
...
def preparing_data(data_dir, tokenizer, max_length):
    ...
    # pad with 0 the sentence  while the words of sentence is less than
    # max_length
...
def pretrain_model(data_dir):
    ...
    train_dataset, valid_dataset, test, target_names = preparing_data(
        data_dir, tokenizer, max_length)
    # load the model using GPU
    model = BertForSequenceClassification.from_pretrained(
        model_name, num_labels=len(target_names)).to(device_ava)
