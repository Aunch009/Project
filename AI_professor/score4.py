...
def init():
    ...
    # Text classification
    classification_main_path = Model.get_model_path(
        'Class_fine_tune', version=6)
    classification_model_path = os.path.join(
        classification_main_path, 'bbc-bert-base-uncased')
    ...
    classification_model = BertForSequenceClassification.from_pretrained(
        classification_model_path, num_labels=len(target_names))
    classification_tokenizer = BertTokenizerFast.from_pretrained(
...
def pos_selected(text):
    ...
    selected_word = [
        word for (
            word,
            pos) in pos_tag(tokenized) if (
            pos in pos_selection) and (
...
def tokenizer(sentence, min_words=4, max_words=200, stemm: str = ''):
    ...
    token = [
        w for w in tokenized if (
            len(w) > min_words and len(w) < max_words and (
                w not in STOPWORDS))]
    return token
...
def BBC_summarization(text, max_length_summarize):
    ...
    tokenized_text = summary_tokenizer.encode(
        t5_prepared_Text, return_tensors="pt").to('cpu')
...
def run(raw_data):
    try:
        ...
        return {
            ...
            'Document summary': result3,
            'Recommadation Document': result2}
